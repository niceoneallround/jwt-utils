...
//
// The JWT payload has the following private claims
// PN_GRAPH holds the data being sent
// PRIVACY_PIPE holds the pipe @id
//
...
jwtUtils.signMetadata = function signMetadata(metadata, jwtOptions, props) {
  ...
  return signUsingPayload(payload, jwtOptions, props);
};
...
//
function signUsingPayload(payload, jwtOptions, props) {
  'use strict';
  ...
  if ((props) && (props.issuer)) { // kind of wierd that can override must be a default somewhere
...
function signUsingHS256(payload, secret, jwtOptions) {
  'use strict';
  assert((jwtOptions.algorithm === 'HS256'), util.format('jwtOptions.algorithm must be HS256 as only type supported:%j', jwtOptions));
