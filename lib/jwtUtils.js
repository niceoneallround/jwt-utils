...
const PN_JWT_TYPE = {
  ...
  subjectQuery: 'https://pn.schema.webshield.io/type#subject_query',
  subjectQueryResult: 'https://pn.schema.webshield.io/type#subject_query_result',
...
// holds an identity syndicate subject query sent from the qpa to the IS
const SUBJECT_QUERY_CLAIM = 'https://pn.schema.webshield.io/prop#subject_query';

// holds an identity syndicate subject query sent from the qpa to the IS
const SUBJECT_QUERY_RESULT_CLAIM = 'https://pn.schema.webshield.io/prop#subject_query_result';
...
jwtUtils.signError = function signError(id, error, jwtOptions) {
  'use strict';
  assert(id, 'signError id param missing');
  assert(error, 'signError error param missing');
  assert(jwtOptions, 'signError jwtOptions param missing');
...
//
// Create a subject query JWT that contains the following claims
// - a subject query claim
// - a privacy pipe claim
//
// *subjectQuery- the jsonld subjct query node - SYNDICATE_REQUEST_CLAIM
// *privacyPipeId - the privacy pipe Id used to send the JWT - PRIVACY_PIPE_CLAIM
// *jwtOptions - control the signing
// *props
//   subject: the subject query @id
//   issuer: comes from the jwtOptions
//
jwtUtils.signSubjectQuery = function signSubjectQuery(query, privacyPipeId, jwtOptions, props) {
  'use strict';
  assert((arguments.length === 4), util.format('signSubjectQuery expected 4 parameters got:%s', arguments.length));
  assert(query, 'signSubjectQuery query param missing');
  assert(privacyPipeId, 'signSubjectQuery privacyPipeId param missing');
  assert(props, 'signSubjectQuery props param missing');
  assert(props.subject, util.format('signSubjectQuery props.subject is missing the query @id', props));

  let payload = {};

  // add the claims to the JWT payload
  payload[PN_JWT_TYPE_CLAIM] = PN_JWT_TYPE.subjectQuery;
  payload[SUBJECT_QUERY_CLAIM] = query;
...
//
// Create a subject query result JWT that contains the following claims
// - QUERY_CLAIM - contains the query result
// - SUBJECT_JWTS_CLAIM - array of subject JWTs used in the query result.
//   Note this may be either the orginal if client can see, or one issued by the IS if can only see some fields
// - PRIVACY_PIPE_CLAIM
// - JWT_TYPE_CLAIM
// - JWT_ID_CLAIM
// - a subject query claim
// - a privacy pipe claim
//
// *subjectQueryResult - the jsonld subject query result
// *subjects - subject data used for results
// *privacyPipeId - the privacy pipe Id used to send the JWT - PRIVACY_PIPE_CLAIM
// *jwtOptions - control the signing
// *props
//   subject: the subject query @id
//   issuer: comes from the jwtOptions
//
jwtUtils.signSubjectQueryResult = function signSubjectQueryResult(queryResult, subjectJWTs, privacyPipeId, jwtOptions, props) {
  'use strict';
  assert((arguments.length === 5), util.format('signSubjectQueryResult expected 4 parameters got:%s', arguments.length));
  assert(queryResult, 'signSubjectQueryResult query param missing');
  assert(subjectJWTs, 'signSubjectQueryResult subjectJWTs param missing');
  assert(privacyPipeId, 'signSubjectQueryResult privacyPipeId param missing');
  assert(props, 'signSubjectQueryResult props param missing');
  assert(props.subject, util.format('signSubjectQueryResult props.subject is missing the query @id', props));

  let payload = {};

  // add the claims to the JWT payload
  payload[PN_JWT_TYPE_CLAIM] = PN_JWT_TYPE.subjectQueryResult;
  payload[SUBJECT_QUERY_RESULT_CLAIM] = queryResult;
...
// - a PRIVACY_PIPE_CLAIM - the pipe that was used to load the pipe into the lake
//
...
jwtUtils.signSubject = function signSubject(subject, pnDataModelId, syndicationId, jwtOptions, props) {
  ...
  assert(props.privacyPipe, util.format('signSubject - props.privacyPipe missing:%j', props)); // subjects always pass down a pipe
...
/**
  Produces a cryptographic hash, SHA256, across the base64 encoded JWT, this
  includes the header, payload, and signature.

  @param jwt the base64 encoded JWT
  @return the cryptographic hash
*/
jwtUtils.SHAJWT = function SHAJWT(jwt) {
  'use strict';
  assert(jwt, 'SHAJWT jwt is missing');
...
module.exports = {
  ...
  claims: {
    ...
    SUBJECT_QUERY_CLAIM: SUBJECT_QUERY_CLAIM,
    SUBJECT_QUERY_RESULT_CLAIM: SUBJECT_QUERY_RESULT_CLAIM,
