...
// holds syndicated entity metadata describing how to build an syndicated entity from
// the subject data. Note JWT needs a subject claim to make any sense.
//
const SYNDICATED_ENTITY_CLAIM = 'https://pn.schema.webshield.io/prop#syndicated_entity';
...
// syndication job id
//
const SUBJECT_SYNDICATION_ID_CLAIM = 'https://pn.schema.webshield.io/prop#subject_syndication_id';
...
// *syndEnts - either a single node or array of nodes - SYNDICATED_ENTITY_CLAIM
...
//   issuer: comes from the jwtOptions
//
jwtUtils.signRSQueryRequest = function signRSQueryRequest(query, syndEnt, subject, privacyPipeId, jwtOptions, props) {
  'use strict';
  assert(query, 'signRSQueryRequest query param missing');
  assert(syndEnt, 'signRSQueryRequest query param missing');
  assert(subject, 'signRSQueryRequest subject param missing');
  assert(privacyPipeId, 'signRSQueryRequest privacyPipeId param missing');
  assert(props, 'signRSQueryRequest props param missing');
  assert(props.subject, util.format('signRSQueryRequest props.subject is missing the syndicate request @id', props));

  let payload = {};

  // add the claims to the JWT payload
  payload[QUERY_CLAIM] = query;
  payload[SYNDICATED_ENTITY_CLAIM] = syndEnt;
...
// - a subject syndication id claim - this contains the @id of the syndication action that generated the JWT
//
...
jwtUtils.signSubject = function signSubject(subject, pnDataModelId, syndicationId, jwtOptions, props) {
  ...
  assert(pnDataModelId, 'signSubject syndicationId param missing');
  ...
  payload[SUBJECT_SYNDICATION_ID_CLAIM] = syndicationId;
  payload[SUBJECT_CLAIM] = subject;
...
module.exports = {
  ...
  claims: {
    ...
    SYNDICATED_ENTITY_CLAIM: SYNDICATED_ENTITY_CLAIM,
    SYNDICATE_REQUEST_CLAIM: SYNDICATE_REQUEST_CLAIM,
    SUBJECT_SYNDICATION_ID_CLAIM: SUBJECT_SYNDICATION_ID_CLAIM,
