...
describe('1 Sign RSP Query Result Tests', function () {
  'use strict';
  ...
  let message = 'fake-message-jwt';

  it('1.1 HS256 - should create a JWT containing a query and massage claim in the payload',
    function () {
      let props = { subject: query['@id'] };

      let token = JWTUtils.signRSPQueryResult(query, message, hs256Options, props);
      ...
      verified.should.have.property(JWTClaims.EMBEDDED_JWT_MESSAGE_CLAIM, message);
      verified.should.have.property(JWTClaims.PN_JWT_TYPE_CLAIM, JWTType.rspQueryResult);
    }); //it 1.1

  it('1.2 RS256 - should should create a JWT containing a subject claim in the payload', function () {

    let props = { subject: query['@id'] };
    let token = JWTUtils.signRSPQueryResult(query, message, rs256Options, props);
    assert(token, 'no token produced');
